...
import { LogoImageRegistButton, LogoImageDeleteButton } from '@/components/ui/buttons/admin/logoImageButton';
import { RiyoKiyakuRegistButton, RiyoKiyakuDeleteButton } from '@/components/ui/buttons/admin/riyoKiyakuButton';
import { PrivacyPolicyRegistButton, PrivacyPolicyDeleteButton } from '@/components/ui/buttons/admin/privacyPolicyButton';
...
const Page: React.FC<PageProps> = ({ kengen }) => {
  ...
  useEffect(() => {
    if (data) {
      setSystemData(data);
      if (data.memberRegistrationFlg) { setSelectedMemberRegistration(data.memberRegistrationFlg.toString()); }
      if (data.memberApprovalFlg) { setSelectedMemberApproval(data.memberApprovalFlg.toString()); }
      if (data.nologinView) { setSelectedNologinView(data.nologinView.toString()); }
      if (data.faviconImagePath) { setFaviconImageFlg(true); }
  ...
  useEffect(() => {
    if (faviconErrors) { setFormErrors(faviconErrors); }
  }, [faviconErrors]);
  ...
  const handleFaviconDelete = useCallback(async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
  ...
  useEffect(() => {
    if (logoErrors) { setFormErrors(logoErrors); }
  }, [logoErrors]);
  ...
  const handleLogoDelete = useCallback(async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();      
    e.stopPropagation(); // イベントの伝播を停止
  ...
  useEffect(() => {
    if (riyoKiyakuErrors) { setFormErrors(riyoKiyakuErrors);}
  }, [riyoKiyakuErrors]);
  ...
  const handleRiyoKiyakuDelete = useCallback(async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    e.stopPropagation(); // イベントの伝播を停止
    await riyoKiyakuDeleteAPI(systemData.systemSeq);
  }, [systemData, riyoKiyakuDeleteAPI]);

  const { privacyPolicyResponseData, privacyPolicyErrors, privacyPolicyRegist } = usePrivacyPolicyRegistAPI();
  useEffect(() => {
    if (privacyPolicyErrors) { setFormErrors(privacyPolicyErrors); }
  }, [privacyPolicyErrors]);
  ...
  const handlePrivacyPolicyDelete = useCallback(async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    e.stopPropagation(); // イベントの伝播を停止
                    ...
                    <>
                      {!faviconImageFlg ? (
                        <>
                          ...
                          />
                          {formErrors?.faviconImage && <p className="error-message">{formErrors.faviconImage}</p>}
                    ...
                    <>
                      {!logoImageFlg ? (
                        <>
                          ...
                          />
                          {formErrors?.logoImage && <p className="error-message">{formErrors.logoImage}</p>}
                    ...
                    <>
                      {!riyoKiyakuFlg ? (
                        <>
                          ...
                          />
                          {formErrors?.riyoKiyaku && <p className="error-message">{formErrors.riyoKiyaku}</p>}
                      ...
                      {!privacyPolicyFlg ? (
                        <>
                          <FileUpload
                            ...
                            allowedExtensions={['pdf']}
                          />
                          {formErrors?.privacyPolicy && <p className="error-message">{formErrors.privacyPolicy}</p>}
                          <div>{texts.system.systemRegist_note_8}</div>
                          <div className="text-right">
