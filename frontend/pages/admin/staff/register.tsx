...
import { StaffTeishiOnButton, StaffTeishiOffButton } from '@/components/ui/buttons/admin/staffTeishiButton';
//スタイル
      ...
      <div className="flex flex-col items-center min-h-screen bg-gray-100">
        <div className="w-full p-8 space-y-6 bg-white shadow-md md:max-w-full md:rounded">
          ...
          { texts.staff.loginId }
              ...
              />
              {formErrors?.loginId && <p  className={`error-message ${styles.input50}`}>{formErrors.loginId}</p>}
            </div>
            <div>
            <div className="flex items-center">
          <RequiredMark />
          ...
          { texts.staff.staffName }
              ...
              className={`${styles.commonInput} ${styles.input50}`}
              />
              {formErrors?.staffName && <p  className={`error-message ${styles.input50}`}>{formErrors.staffName}</p>}
            </div>
            <div>
            <div className="flex items-center">
          <RequiredMark />
          ...
          { texts.common.mail }
              ...
              className={`${styles.commonInput} ${styles.input50}`}
              />
              {formErrors?.mail && <p  className={`error-message ${styles.input50}`}>{formErrors.mail}</p>}
            </div>
              ...
              <div>
                <KengenListPullDown
                 className={`${styles.commonInput} ${styles.input50}`}
                  ...
                  selectedId={
                    staff.kengenId === '0' || staff.kengenId === null || staff.kengenId === undefined
                      ? ''
                      : String(staff.kengenId)
                  }
                />
              </div>
              {formErrors?.kengenId && <p  className={`error-message ${styles.input50}`}>{formErrors.kengenId}</p>}
            </div>
