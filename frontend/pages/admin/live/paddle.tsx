...
const Page: React.FC<PageProps> = ({ kengen }) => {
  ...
  const handleUpdateChange = (
    userId: string,
    field: keyof TAdminPaddleRegistRequest,
    value: string
  ) => {
    setfetchedData((prevData) =>
      prevData.map((item) => (item.userId === userId ? { ...item, [field]: value } : item))
    );
  };
  ...
  useEffect(() => {
    if (updateResponseErrors) {
      setUpdateErrors(updateResponseErrors);
    }
  }, [updateResponseErrors]);

  //承認
  const {
    responseStatus: onlineBidShoninResponseStatus,
    errors: onlineBidShoninResponseErrors,
    onlineBidShoninAPI,
                  ...
                  <tr key={result.userId} className="cursor-pointer hover:bg-gray-100">
                    ...
                    <td className="py-1 px-4 border-b w-40">
                      <input
                        ...
                        onChange={(e) =>
                          handleUpdateChange(result.userId, "paddleNo", e.target.value)
                        }
                        className={adminStyles.tabelCellInput}
                      />
                    ...
                    <td className="py-1 px-4 border-b text-center">
                      {executionPermission(353, 2) ? (
                        <RegistButton
                          ...
                          onClick={() => paddleDataUpdate(result)}
                        />
                      ) : (
                        <></>
                      )}
