...
export const useTorihikiJissekiCsvAPI = () => {  
  ...
  const torihikiJissekiCsv  = async (auctionSeq: number, selectedUserIds: number[], csvKbn: number)=>{
   
    const baseEndpoint = csvKbn === 1 ? 'torihikiJisseki/torihikiJissekicsv' : 'torihikiJisseki/meisaicsv';
