...
export const useTorihikiJissekiMeisaiDateUpdateAPI = () => {
  ...
  const [rakusatsuResponseData, setRakusatsuResponseData] = useState<
    TTorihikiJissekiMeisaiRakusatsuSelect[]
  >([]);
  const [shuppinResponseData, setShuppinResponseData] = useState<
    TTorihikiJissekiMeisaiShuppinSelect[]
  >([]);
  const torihikiJissekiMeisaiDateUpdateAPI = async (
