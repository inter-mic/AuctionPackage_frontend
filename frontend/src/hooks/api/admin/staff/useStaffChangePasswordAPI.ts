...
export const useReissuePasswordAPI =  ()  => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const endPoint = `staff/changePassword`;
    const requestBody = { oldPassword, newPassword, newPasswordConfirm};
    const { status, data: responseData } = await apiRequest("admin", endPoint, 'POST', requestBody, texts.message.changePassword, true);
    if (status == 400) {
