...
export const usePageSettingRegistAPI = () => {
  ...
  const pageSettingRegistAPI = async (data: TTtPageSetting,  file: File | null) => {
    ...
    const { status, data: responseData } = await apiRequest("admin", `pageSetting/update/${data.pageSeq}`, 'POST', formData, texts.message.regist, true);
    if (status == 400) {
