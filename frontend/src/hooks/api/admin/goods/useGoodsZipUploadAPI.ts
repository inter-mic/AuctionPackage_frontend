...
export const useGoodsZipUploadAPI = () => {
  ...
  const goodsZipUpload = async (zipUpdateData: ZipUpdateData, file: File | null) => {
    ...
    formData.append('zipUploadData', new Blob([JSON.stringify(zipUpdateData)], {type : 'application/json'}))
    ...
    const { status, data: responseData } = await apiRequest("admin", endPoint, 'POST', formData, texts.message.regist, true, {}, {
      onUploadProgress: (event: AxiosProgressEvent) => { // ✅ `AxiosProgressEvent` を使用
        if (event.total) {
