...
export const useMessageRegistAPI = () => {
  ...
  const messageRegistAPI = async (messageSeq: any, message: any)=>{
    const endPoint = messageSeq
      ? `liveMessage/update/${messageSeq}`
      : 'liveMessage/insert';
    const { status, data: responseData } = await apiRequest("admin", endPoint, 'POST', message, texts.message.regist, false);
    if (status == 400) {
