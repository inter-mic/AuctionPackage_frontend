...
export const useLiveBidKekkaUpdateAPI = () => {
  ...
  const liveBidKekkaUpdateAPI = async (liveBidKekka: LiveBidKekkaData, liveBidLog: TLiveBidLog[], connectionCount: number | null | undefined) => {
    ...
    formData.append('liveBidKekkaData', new Blob([JSON.stringify(sanitizedKekkaData)], {type : 'application/json'}))

    const { status, data: responseData } = await apiRequest("admin", `liveBid/kekkaUpdate/${liveBidKekka.goodsId}`, 'POST', formData, texts.message.regist, true);
    if (status === 400) {
