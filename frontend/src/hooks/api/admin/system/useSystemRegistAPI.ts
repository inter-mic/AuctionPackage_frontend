...
export const useSystemRegistAPI = () => {
  ...
  const systemRegist = async (SystemData: any) => {
    ...
    const endPoint = systemSeq
                    ? `system/update/${systemSeq}`
                    : 'system/insert';

    const { status, data: responseData } = await apiRequest("admin", endPoint, 'POST', SystemData, texts.message.regist, true);
    if (status == 400) {
