...
export const useGoodsSearchBeforeAfterLotAPI = () => {
  ...
  const goodsSearchBeforeAfterLotAPI = async (auctionSeq: number, lot: string, isLogin: boolean) => {
    const endPointKbn = `${isLogin ? "member" : "public"}`;
    const params = {
      auctionSeq: auctionSeq,
      lot: lot,
  };
    const { status, data: responseData } = await apiRequest(endPointKbn, "goods/searchBeforeAfterLot", 'POST', params, "", true);
