...
export const useResultsSearchAPI = () => {
  ...
  const registesultsSearchAPI = async (searchParams: TResultsSearchRequest) => {
    const { status, data: responseData } = await apiRequest("member", "results/search", 'POST', searchParams, "", true);
    if (status == 400) {
  ...
  return { resultsList, errors, registesultsSearchAPI }
};