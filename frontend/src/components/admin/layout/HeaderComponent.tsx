    ...
    <>
      <header className={styles.header}>
        ...
        <nav className={styles.menu}>
          <ul ref={dropdownRef}>
            <li className={styles.toggleMenu} onClick={() => toggleSubMenu("member")}>
            ...
            {isSubMenusOpen["member"] && (
              <ul className={styles.adminDropdown}>
                {hasPermission(kengen, 101) && (
                  <li>
                    <Link href="/admin/member/register">{texts.menu.adminMemberRegist}</Link>
                  </li>
                )}
                {hasPermission(kengen, 102) && (
                  <li>
                    <Link href="/admin/member/search">{texts.menu.adminMemberList}</Link>
                  </li>
                )}
                {hasPermission(kengen, 103) && (
                  <li>
                    <Link href="/admin/member/addinfoItemRegister">
                ...
                {hasPermission(kengen, 104) && (
                  <li>
                    <Link href="/admin/member/torihikijisseki/search">
                      {texts.menu.adminTorihikiJisseki}
                    </Link>
                  </li>
                )}
              </ul>
            )}
            <li className={styles.toggleMenu} onClick={() => toggleSubMenu("goods")}>
            ...
            {isSubMenusOpen["goods"] && (
              <ul className={styles.adminDropdown}>
                {hasPermission(kengen, 201) && (
                  <li>
                    <Link href="/admin/goods/register">{texts.menu.adminGoodsRegist}</Link>
                  </li>
                )}
                {hasPermission(kengen, 202) && (
                  <li>
                    <Link href="/admin/goods/search">{texts.menu.adminGoodsList}</Link>
                  </li>
                )}
                {hasPermission(kengen, 203) && (
                  <li>
                    <Link href="/admin/goods/bulkRegister">{texts.menu.adminGoodsBulkRegist}</Link>
                  </li>
                )}
                {hasPermission(kengen, 206) && (
                  <li>
                    <Link href="/admin/goods/addinfoItemRegister">
                ...
                {hasPermission(kengen, 207) && (
                  <li>
                    <Link href="/admin/goods/category">{texts.menu.adminCategoryRegist}</Link>
                  </li>
                )}
              </ul>
            )}
            <li className={styles.toggleMenu} onClick={() => toggleSubMenu("auction")}>
            ...
            {isSubMenusOpen["staff"] && (
              <ul className={styles.adminDropdown}>
                {hasPermission(kengen, 401) && (
                  <li>
                    <Link href="/admin/staff/register">{texts.menu.adminStaffRegist}</Link>
                  </li>
                )}
                {hasPermission(kengen, 402) && (
                  <li>
                    <Link href="/admin/staff/search">{texts.menu.adminStaffList}</Link>
                  </li>
                )}
                {hasPermission(kengen, 403) && (
                  <li>
                    <Link href="/admin/staff/kengen">{texts.menu.adminKengenRegist}</Link>
                  </li>
                )}
              </ul>
            )}
            <li className={styles.toggleMenu} onClick={() => toggleSubMenu("setting")}>
            ...
            {isSubMenusOpen["setting"] && (
              <ul className={styles.adminDropdown}>
                {hasPermission(kengen, 501) && (
                  <li>
                    <Link href="/admin/setting/info/register">{texts.menu.adminInfoRegist}</Link>
                  </li>
                )}
                ...
                {hasPermission(kengen, 504) && (
                  <li>
                    <Link href="/admin/setting/admin/register">{texts.menu.adminAdminRegist}</Link>
                  </li>
                )}
                {hasPermission(kengen, 505) && (
                  <li>
                    <Link href="/admin/setting/mail/register">{texts.menu.adminMailRegist}</Link>
                  </li>
                )}
                {hasPermission(kengen, 506) && (
                  <li>
                    <Link href="/admin/setting/page/register">{texts.menu.adminPageSetting}</Link>
                  </li>
                )}
              </ul>
            )}
        ...
        <nav className={styles.menu}>
          <ul ref={dropdownRef}>
            <li className={styles.toggleMenu} onClick={() => toggleSubMenu("mypage")}>
              {userName || ""}様
            </li>
            {isSubMenusOpen["mypage"] && (
              <ul className={styles.adminDropdown}>
                <li>
                  <Link href="/admin/changePassword">{texts.menu.changePassword}</Link>
                </li>
              </ul>
            )}
          </ul>
          <li onClick={handleLogout} className={`${styles.logoutBtn} ${styles.toggleMenu}`}>
            {texts.button.logout}
          </li>
        </nav>
                ...
                <li>
                  ...
                  {isSubMenusOpen["goods"] && (
                    <ul className={styles.subMenu}>
                      ...
                      {hasPermission(kengen, 203) && (
                        <li>
                          <Link href="/admin/goods/bulkRegister">
                            {texts.menu.adminGoodsBulkRegist}
                          </Link>
                        </li>
                      )}
                      {hasPermission(kengen, 206) && (
                      ...
                      {hasPermission(kengen, 207) && (
                        <li>
                          <Link href="/admin/category/register">
                            {texts.menu.adminCategoryRegist}
                          </Link>
                        </li>
                      )}
                    </ul>
                  ...
                  {isSubMenusOpen["staff"] && (
                    <ul className={styles.subMenu}>
                      ...
                      {hasPermission(kengen, 403) && (
                        <li>
                          <Link href="/admin/kengen/register">{texts.menu.adminKengenRegist}</Link>
                        </li>
                      )}
                    </ul>
                  )}
