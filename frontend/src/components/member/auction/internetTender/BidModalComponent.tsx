...
const BidModalComponent: React.FC<Props> = ({
  ...
  const uniqueBidUnitList = React.useMemo(() => {
    ...
    return auctionBidUnitList.filter(unit => {
  ...
  const isUnitButtonEnabled = useCallback((unitValue: number, isIncrease: boolean): boolean => {
    ...
    return false;
  }, [currentBid, initialPrice, auctionBidUnitList]);
  ...
  useEffect(() => {
    if (auctionBidErrors) {
      setFormErrors(auctionBidErrors);
    }
    if (tenderBidErrors) {
      setFormErrors(tenderBidErrors);
    }
  }, [auctionBidErrors, tenderBidErrors]);
  useEffect(() => {
    if (auctionBidResponseStatus === 200) {
      handleToggleFilter();
    }
  ...
  useEffect(() => {
    if (auctionBidResponseStatus) {
      if (tenderBidResponseStatus == 200) {
        handleToggleFilter();
      }
    }
    if (tenderBidResponseStatus) {
      if (tenderBidResponseStatus == 200) {
        handleToggleFilter();
      }
    }
