...
const AuctionInfoComponent: React.FC<Props> = ({ auctionData, isToGoodsList, isLogin }) => {
  ...
  const displayStarttimeFormatted = dayjs(auctionData.displayStarttime).format(
    "YYYY年MM月DD日 H:mm"
  );
  ...
  const bidStart = auctionData.bidStarttime
    ? dayjs(auctionData.bidStarttime, "YYYY/MM/DD HH:mm:ss")
    : null;
  const bidEnd = auctionData.bidEndtime
    ? dayjs(auctionData.bidEndtime, "YYYY/MM/DD HH:mm:ss")
    : null;
    ...
    <div className={auctionStyles.auctionContainer}>
      <div className={`${memberStyles.memberContainer} ${auctionStyles.auctionBlock}`}>
        ...
        <div className={auctionStyles.textContainer}>
          ...
          <h2 className={auctionStyles.auctionName}>{auctionData.auctionName}</h2>
          ...
          {auctionData.auctionListUrl != null && (
            <div className={auctionStyles.goodsList}>
              ...
              <Link
                ...
                href={auctionData.auctionListUrl}
                target="_blank"
              >
          ...
          <div className={auctionStyles.buttonContainer}>
            ...
            {auctionData?.spnKbn === "1" &&
              ...
              dayjs().isBefore(dayjs(auctionData.onlinebidApplicationEndtime)) &&
              (auctionData.paddleExists ? (
                <span className={auctionStyles.liveApplication}>
          ...
          {auctionData.auctionGaiyo && (
            <>
              <button onClick={toggleExpand} className={auctionStyles.toggleButton}>
                {isExpanded ? texts.auction.auctionGaiyoClose : texts.auction.auctionGaiyoOpen}
              </button>
              <div
                className={`${auctionStyles.auctionGaiyo} ${
                  isExpanded ? auctionStyles.auctionGaiyoExpanded : ""
                }`}
              >
                <div
                  className={`${auctionStyles.auctionGaiyo} ${
                  ...
                  dangerouslySetInnerHTML={{
                    __html: auctionData.auctionGaiyo.replace(/(\r\n|\n|\r)/g, "<br>"),
                  }}
                />
              </div>
            </>
      ...
      <LiveApplicationModalComponent
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        auctionSeq={auctionData.auctionSeq || 0}
