...
const ImageThumbnail: React.FC<SortableItemProps> = ({ id, children, linkUrl, onChange, onDelete }) => {
    ...
    return (
        <div className="w-1/3 p-2">
            ...
            />
            <button style={buttonStyle} onClick={handleDeleteClick}>削除</button>
        </div>
    );
};
...
export const ImageTopList: React.FC<ImageSortableListProps> = ({ images, onImagesUpdate }) => {
    ...
    const handleChange = (id: string, e: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
    ...
    return (
        <div>
            <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext items={images.map((image) => image.imageSeq)} strategy={verticalListSortingStrategy}>
