...
interface RegistButtonProps {
  ...
  setFormUpdateErrors: React.Dispatch<React.SetStateAction<{ [key: string]: { [key: string]: string } }>>;
}
export const CategoryUpdateButton: React.FC<RegistButtonProps> = ({ categorySeq, categoryName, setFormUpdateErrors }) => {
  const { useState, useEffect, useCallback, texts } = useCommonSetup();
    ...
    return (
        <button onClick={handleClick} className="bg-yellow-500 hover:bg-opacity-50 text-white font-bold py-2 px-4 rounded-lg  w-full sm:w-40" >
